...
function init() {
    ...
    commandButton = document.getElementById('command-btn');
    output = document.getElementById('convertedText');

    let voice_recording = false;
    recordButton.onclick = function () {
        ...
        if (!voice_recording) {
            recognition.start();
            recordButton.innerHTML = '錄音中...';
            console.log('Ready to receive a command.');
            voice_recording = true;
        } else {
            recognition.stop();
            recordButton.innerHTML = '錄製輸入';
            voice_recording = false;
        }
    }

    let command_recording = false;
    commandButton.onclick = function () {
        ...
        if (!command_recording) {
            recognition.start();
            commandButton.innerHTML = '錄音中...';
            console.log('Ready to receive a command.');
            command_recording = true;
        } else {
            recognition.stop();
            commandButton.innerHTML = '錄製指令';
    ...
    recognition.onresult = function (event) {
        ...
        } else if (command_recording) {
            if (isFinal){
                sendDataToFlask({
                    'original_text': output.textContent,
                    'command': text
                });
            }
...
function sendDataToFlask(data) {
    ...
    .then(result => {
        ...
        console.log(result);
        output.textContent = result.new_text;
    })
    .catch(error => {
